...
1. **Невалідний вивід LLM:** Використання `response_schema` на рівні API Gemini гарантує відповідність структурі JSON. При помилках валідації або збоях мережі виконується до 3 повторних спроб (Tenacity). При остаточній помилці запис маркується як помилковий без зупинки всього процесу.
2. **Ліміти API:** Ковзний лімітер швидкості (Sliding Window) контролює RPM та TPM, запобігаючи помилкам HTTP 429. Робота з семафором обмежує конкурентність до 5 запитів.
3. **Збереження прогресу:** Прогрес фіксується в реальному часі у файлі `progress.json`, що дозволяє продовжити роботу з місця зупинки при збоях.
